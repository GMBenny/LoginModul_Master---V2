...
namespace LoginModul
{
    public class MsSqlLoginLogDataMapper : MsSqlConnect, ILoginLogDataMapper
    {
        ...
        public void Create(LoginLog data)
        {
            ...
            ConnectDB();
            sqlCommandString = "insert into LoginLog(LoginLogID,Timestamp,Location,Severity,Message) values (" + data.LoginLogID.ToString() + ",'" + data.LogTime.ToString("yyyy-MM-dd HH':'mm':'ss'.'fffffff") + "','" + data.Location + "','" + data.Severity + "', '" + data.Message + "')";
